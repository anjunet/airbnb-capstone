...
const Dashboard = () => {
  ...
  // Handles date changes and calculates the number of nights
  const handleDateChange = (e) => {
    const { name, value } = e.target;
    const newParams = { ...searchParams, [name]: value };
  
    if (newParams.checkIn && newParams.checkOut) {
      const checkInDate = new Date(newParams.checkIn);
      const checkOutDate = new Date(newParams.checkOut);
      const nights = (checkOutDate - checkInDate) / (1000 * 60 * 60 * 24);
      newParams.nights = nights > 0 ? nights : 0;
    }
  
    setSearchParams(newParams);
  };
  
  // Redirects to the listings page with selected filters
  const handleSearch = () => {
  ...
  return (
    <DashboardContainer>
      <Header>
        <HeaderContainer>
          <HeaderLeft>
            <Logo src=" https://1000logos.net/wp-content/uploads/2017/08/Airbnb-Logo-768x432.png" />
          </HeaderLeft>
        ...
        <SearchBar>
          ...
          <SearchField>
            ...
            {searchParams.nights > 0 && (
              <small>{searchParams.nights} nights</small>
            )}
          </SearchField>
